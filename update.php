...
if(isset($_POST['btnUpdate'])){
	...
	if($mysqli->connect_errno) {
		die("Failed connecting to MySQL: ".$mysqli->connect_errno);
	}
	...
	if (!$stmt->execute()){
		die("Error updating patrolcar table: ".$stmt->errno);
	}
	...
	if ($_POST["patrolCarStatus"] == '4') {
		...
		if(!$stmt->execute()){
			die("Error updating dispatch table: ".$stmt->errno);
		}
...
<?php }
	...
	if(!($stmt = $mysqli->prepare($sql))) {
		die("Error Preparing: ".$mysqli->errno);
	}
